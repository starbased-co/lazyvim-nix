...
local NAMING_PATTERNS = {
  ...
  { pattern = "(.+)/(.+)%.nvim$", transform = function(owner, name) return name .. "-nvim" end },
  { pattern = "(.+)/(.+)%-nvim$", transform = function(owner, name) return name .. "-nvim" end },
  { pattern = "(.+)/nvim%-(.+)$", transform = function(owner, name) return "nvim-" .. name end },
  { pattern = "(.+)/(.+)$", transform = function(owner, name) return name end },
}
...
function M.verify_nixpkgs_package(package_name)
  ...
  local cmd = string.format("nix eval --impure --expr 'let pkgs = import <nixpkgs> {}; in pkgs.vimPlugins.%s or null' 2>/dev/null", package_name)
    ...
    if owner_or_base and module then
      ...
      if base_package then
        return {
          ...
          module_name = module,
          base_repo = base_key == "echasnovski/mini" and "echasnovski/mini.nvim" or (owner_or_base .. "/" .. module:match("^(.+)%.") or owner_or_base),
          confidence = "high",
  ...
  for _, candidate in ipairs(analysis.multi_module_candidates) do
    if candidate.multi_module.confidence == "high" then
      table.insert(updates, {
        ...
        line = string.format('  "%s" = { package = "%s"; module = "%s"; };', 
                           candidate.name, 
